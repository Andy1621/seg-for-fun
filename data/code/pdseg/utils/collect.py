...
class SegConfig(dict):
    ...
    def check_and_infer(self):
        ...
        test_sets = codecs.open(self.DATASET.TEST_FILE_LIST, 'r',
                                'utf-8').readlines()
        vis_sets = codecs.open(self.DATASET.VIS_FILE_LIST, 'r',
                                'utf-8').readlines()
        ...
        self.DATASET.VIS_TOTAL_IMAGES = len(vis_sets)
        if self.DATASET.NEGETIVE_TRAIN_FILE_LIST is not None:
            negetive_train_sets = codecs.open(self.DATASET.NEGETIVE_TRAIN_FILE_LIST, 'r',
                                    'utf-8').readlines()
            negetive_length = len(negetive_train_sets)
            positive_length = len(train_sets)
            if int(positive_length * self.DATASET.NEGETIVE_RATIO) < negetive_length:
                negetive_length = int(positive_length * self.DATASET.NEGETIVE_RATIO)
            self.DATASET.TRAIN_TOTAL_IMAGES += negetive_length
