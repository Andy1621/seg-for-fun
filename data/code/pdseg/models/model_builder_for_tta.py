...
def flip_combination():
    """
    Get flip combination.
    """

    flip_comb = [(False, False)]
    if cfg.TEST.HORIZAONTAL_FLIP:
        flip_comb.append((True, False))
    if cfg.TEST.VERTICAL_FLIP:
...
def tta(image, class_num):
    final_logit = 0
    h_input, w_input = image.shape[-2], image.shape[-1]
    flip_comb = flip_combination()
    for scale in cfg.TEST.SCALES:
        h = int(h_input * scale + 0.5)
        w = int(w_input * scale + 0.5)
        image = fluid.layers.interpolate(image, (h, w), resample='BILINEAR')
        for flip in flip_comb:
            image_flip = tensor_flip(image, flip)
            ...
            logit = fluid.layers.interpolate(logit, (h_input, w_input), resample='BILINEAR')
    ...
    with fluid.program_guard(main_prog, start_prog):
        with fluid.unique_name.guard():
            ...
            if ModelPhase.is_train(phase) or ModelPhase.is_eval(phase):
                ...
                '''
                # 增加二分类focal loss，陈庆 changed on 2020/11/07
                if "focal_loss" in loss_type:
                    avg_loss_list.append(multi_focal_loss(logits, label, mask))
                    loss_valid = True
                    valid_loss.append("focal_loss")
                '''
                if not loss_valid:
                    raise Exception(
