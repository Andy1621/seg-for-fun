...
from models.model_builder_for_tta import build_model
from models.model_builder_for_tta import ModelPhase
...
def visualize(cfg,
    ...
    test_reader = dataset.batch(dataset.generator, batch_size=cfg.TEST.BATCH_SIZE, is_test=True)
    img_cnt = 0
    total_logit = None
    test_reader = tqdm(test_reader, total=math.ceil(cfg.DATASET.VIS_TOTAL_IMAGES // cfg.TEST.BATCH_SIZE), ncols=120)
    ...
    # store model logit
    # with open(os.path.join(save_dir, 'total_logit'), 'wb') as f:
    #     pickle.dump(total_logit, f)
    # print("Save logit in", os.path.join(save_dir, 'total_logit'))
