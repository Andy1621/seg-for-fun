...
def load_checkpoint(exe, program):
    ...
    # If resume model path is end of digit, restore epoch status
        ...
        begin_epoch = epoch + 1
    # 存储最近模型，方便resume
    elif epoch_name == 'latest':
        epoch_file = os.path.join(model_path, 'epoch.txt')
        with open(epoch_file, 'r') as f:
            epoch = int(f.read().split('\n')[0])
        begin_epoch = epoch + 1
    elif epoch_name == 'best_model':
        epoch_file = os.path.join(model_path, 'epoch.txt')
        with open(epoch_file, 'r') as f:
            epoch = int(f.read().split('\n')[0])
        begin_epoch = epoch + 1
...
def update_best_model(ckpt_dir, model_name='best_model'):
    best_model_dir = os.path.join(cfg.TRAIN.MODEL_SAVE_DIR, model_name)
...
def train(cfg):
    ...
    if cfg.DATASET.TRAIN_TOTAL_IMAGES % cfg.BATCH_SIZE and drop_last != True:
        all_step += 1
    step = all_step * (begin_epoch - 1)
    all_step *= cfg.SOLVER.NUM_EPOCHS

    avg_loss = 0.0
    best_mIoU = 0.0
    if cfg.TRAIN.CLASS_NUM:
        best_cIoU = [0.0] * cfg.DATASET.NUM_CLASSES
    ...
    for epoch in range(begin_epoch, cfg.SOLVER.NUM_EPOCHS + 1):
        ...
        if (epoch % cfg.TRAIN.SNAPSHOT_EPOCH == 0
            ...
            ckpt_dir = save_checkpoint(train_prog, epoch, epoch=epoch)
            save_infer_program(test_prog, ckpt_dir)

        # Save the latest epoch for eval
        if cfg.TRAINER_ID == 0:
            ckpt_dir = save_checkpoint(train_prog, 'latest', epoch=epoch)
            save_infer_program(test_prog, ckpt_dir)
            if args.do_eval:
                print("Evaluation start")
                category_iou, mean_iou, _, mean_acc = evaluate(
                    cfg=cfg,
                    ckpt_dir=ckpt_dir,
                ...
                if cfg.TRAIN.CLASS_NUM is None:
                    ...
                    if mean_iou > best_mIoU:
                        ...
                        print_info("Save best model {} to {}, mIoU = {:.4f}".format(
                            ...
                            os.path.join(cfg.TRAIN.MODEL_SAVE_DIR, 'best_model'),
                            mean_iou))
                    for class_num in cfg.TRAIN.CLASS_NUM:
                        class_iou = category_iou[class_num]
                        if class_iou > best_cIoU[class_num]:
                            best_cIoU[class_num] = class_iou
                            update_best_model(ckpt_dir, model_name='best_model_class{}'.format(class_num))
                            print_info("Save best model {} to {}, class{}_iou = {:.4f}".format(
                                ckpt_dir,
                                os.path.join(cfg.TRAIN.MODEL_SAVE_DIR, 
                                            'best_model_class{}'.format(class_num)),
                                class_num,
                                class_iou))

            # Use VisualDL to visualize results
            if args.use_vdl and args.vdl_result and cfg.DATASET.VIS_FILE_LIST is not None:
                visualize(
                    cfg=cfg,
                    use_gpu=args.use_gpu,
    ...
    # save final model
    if cfg.TRAINER_ID == 0:
        ckpt_dir = save_checkpoint(train_prog, 'final', epoch=cfg.SOLVER.NUM_EPOCHS)
        save_infer_program(test_prog, ckpt_dir)
