...
def focal_loss(logit,
    ...
    if weight is None:
        loss, probs = fluid.layers.sigmoid_focal_loss(
            logit,
            label,
            ignore_index=cfg.DATASET.IGNORE_INDEX,
            return_softmax=True)
...
def multi_focal_loss(logits,
    ...
    if isinstance(logits, tuple):
        ...
        for i, logit in enumerate(logits):
            ...
            logit_mask = (logit_label.astype('int32') !=
                          cfg.DATASET.IGNORE_INDEX).astype('int32')
            loss = focal_loss(logit, logit_label, logit_mask, num_classes, weight=weight)
            avg_loss += cfg.MODEL.MULTI_LOSS_WEIGHT[i] * loss
    else:
        avg_loss = focal_loss(
            logits, label, ignore_mask, num_classes, weight=weight)
